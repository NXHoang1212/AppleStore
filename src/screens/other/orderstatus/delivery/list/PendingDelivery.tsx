                        ...
                        <TouchableOpacity style={IndexStyles.StylePendingDelivery.containerItem} onPress={() => navigation.navigate('StackMisc', { screen: 'DetailPendingDelivery', params: { id: item._id, payment: payment } })}>
                            ...
                            <View style={IndexStyles.StylePendingDelivery.viewCart}>
                                <View style={IndexStyles.StylePendingDelivery.viewImagge}>
                                    <Image source={{ uri: item.cart[0].products.priceColor.image }} style={IndexStyles.StylePendingDelivery.image} />
                                </View>
                                <View style={IndexStyles.StylePendingDelivery.viewInfor}>
                                    <Text style={IndexStyles.StylePendingDelivery.textInfor}>{item.cart[0].products.name} {item.cart[0].products.model} {item.cart[0].products.storage}</Text>
                                    <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                                        <Text style={IndexStyles.StylePendingDelivery.textInfor}>{item.cart[0].products.priceColor.color}</Text>
                                        <Text style={IndexStyles.StylePendingDelivery.textQuantity}>x{item.cart[0].quantity}</Text>
                                    </View>
                                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: Responsive.wp(10) }}>
                                        ...
                                        <Text style={IndexStyles.StylePendingDelivery.textPrice}>{FormatPrice(item.cart[0].products.priceColor.price)}</Text>
                                    </View>
                                </View>
                            </View>
                            ...
                            <View style={IndexStyles.StylePendingDelivery.viewLengthCart}>
                                <Text style={IndexStyles.StylePendingDelivery.textLengthCart}>{item.cart.length} sản phẩm</Text>
